(function( $ ) {
	...
	function tableToCSV($table) {
		...
		saveFile("query_results_"+ts+".csv", csv, "application/csv;charset=utf-8");
    }
	...
	function check_sql_query() {
		if ( $('#query').val().match( /\s*(alter|create|drop|rename|insert|delete|update|replace|truncate) /i ) ) {
			return confirm( "No UNDO is available for data modification. Do you want to continue?" );
		} else {
			return true;
		}
	}
		...
		$.post(ajaxurl, data, function(response) {
			if(response.success) {
				...
				if (typeof response.data.affected_rows !== 'undefined') {					
					$('#results_detail').html(response.data.affected_rows + ' row(s) affected');
					$('#results').show();
				} else {	
					if (response.data.rows == 0) {
						...
						$('#results_detail').html('');
						$.jsontotable(response.data.rows, { id: '#results_detail', header: true });
						$("#export_button").show();
					}
					$('#results').show();
				}
	...
	$(function() {
    	$('#table').on('change', function() {
			$('#query').val('describe ' + this.value);
			run_the_sql_query();
		});

		$('#run_the_query_button').on('click', function() {
			if(check_sql_query()) {
				run_the_sql_query();
			}
		});
